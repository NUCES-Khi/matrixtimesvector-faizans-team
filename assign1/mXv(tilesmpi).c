...
void matVecMult(double **matrix, double *vector, double *result, int rows, int cols, int local_rows) {
...
int main(int argc, char *argv[]) {
    ...
    MPI_Scatter(matrix[0], local_rows * size, MPI_DOUBLE, matrix[0], local_rows * size, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    // Perform local matrix-vector multiplication
    matVecMult(matrix, vector, local_result, size, size, local_rows);
